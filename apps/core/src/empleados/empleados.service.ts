...
export class EmpleadosService {
  ...
  async allEmpleados(res: Response) {
    const transaction = await this.sequelize.transaction();
    try {
      const queryResult = await this.sequelize.query(`SELECT
          us.id,
          ue.nombre, 
          ue.apellido1, 
          ue.apellido2,
          (current_date::text || ' ' || ue.fecha_reg::text) as fecha_reg,
          ue.correo, 
          us.usuario, 
          us.tipo  as rol,
          us.estado
        FROM usu_info_emp ue
        JOIN usu_sys us ON us.id = ue.id_usu;`,
        {
